...
FILE_HEADERS_IN_DATASET = list(map(lambda f: f[:-4], os.listdir(os.path.join(DATA_PATH, DATASET, "images"))))
...
for i in range(NUM_FILES_IN_DATASET):
   image = cv2.imread(os.path.join(DATA_PATH, DATASET, "images", FILE_HEADERS_IN_DATASET[i] + ".jpg"), cv2.IMREAD_COLOR)

   with open(os.path.join(DATA_PATH, DATASET, "labels", FILE_HEADERS_IN_DATASET[i] + ".txt"), "r") as boxes:
